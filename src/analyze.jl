...
function all_source_states(state_machine::StateMachine)
    res = Vector{Symbol}()
    for key in keys(state_machine.states)
        if (length(preceeding_transitions(state_machine, key)) == 0) &&
            (length(next_transitions(state_machine, key)) > 0)
            push!(res, key)
        end
    end
    return res
...
function all_sink_states(state_machine::StateMachine)
    res = Vector{Symbol}()
    for key in keys(state_machine.states)
        if (length(next_transitions(state_machine, key)) == 0) &&
            (length(preceeding_transitions(state_machine, key)) > 0)
            push!(res, key)
        end
    end
    return res
...
function all_unconnected_states(state_machine::StateMachine)
    res = Vector{Symbol}()
    for key in keys(state_machine.states)
        (length(adjacent_transitions(state_machine, key)) == 0) && push!(res, key)
    end
    return res
end
